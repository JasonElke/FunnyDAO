...
export const changeApproval = createAsyncThunk(
  ...
  async (
    { bond, provider, networkID, address }: IChangeApproval,
    { dispatch },
  ) => {
    if (!provider) {
...
export const calcBondDetails = createAsyncThunk(
  ...
  async (
    ...
    const vestingTerm = (bond.name === "weth") || (bond.name === "tethys") || (bond.name === "wmetis") ? "432000" : (await bondContract.terms())[1].toString()
    ...
    if(!bond.isClosed){
      try {
        if ((bond.bondToken === "WETH") || (bond.bondToken === "METIS")|| (bond.bondToken === "TETHYS")) {
          ...
          bondDiscount = (marketPrice * Math.pow(10, (bond.decimals == undefined ? 18 : bond.decimals)) - bondPrice) / bondPrice
        }
      } catch (e) {
    ...
    if(!bond.isClosed){
      ...
      } else if (bond.name !== usdc.name && bond.name !== usdt.name) {
    ...
    return {
      ...
      maxBondPrice,
      bondPrice: bondPrice / Math.pow(10, (bond.decimals == undefined ? 18 : bond.decimals)),
      marketPrice,
      maxBondPriceToken,
    }
  },
...
export const bondAsset = createAsyncThunk(
  ...
  async (
    ...
    try {
      ...
      await sleep(10)
      await dispatch(
        calculateUserBondDetails({ address, bond, networkID, provider }),
      )
  ...
  async (
    ...
    try {
      ...
      const pendingTxnType =
        'redeem_bond_' + bond.name + (autostake === true ? '_autostake' : '')
      dispatch(
        fetchPendingTxns({
