...
export class LPBond extends Bond {
    ...
    public getMaiaAmount(networkID: Networks, provider: StaticJsonRpcProvider) {
    ...
    private async getReserves(networkID: Networks, provider: StaticJsonRpcProvider, isToken: boolean): Promise<number> {
        ...
        const isMaia = token1.toLowerCase() === addresses.MAIA_ADDRESS.toLowerCase();

        return isToken ? this.toTokenDecimal(false, isMaia ? reserve0 : reserve1) : this.toTokenDecimal(true, isMaia ? reserve1 : reserve0);
    }
