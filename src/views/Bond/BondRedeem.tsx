        ...
        <Box display="flex" flexDirection="column">
            <Box display="flex" justifyContent="space-around" flexWrap="wrap">
                ...
                <div
                    className="transaction-button bond-approve-btn"
                    onClick={() => {
                        if (bond.pendingPayout < 1) {
                            store.dispatch(warning({ text: 'You can only claim and stake with reward much greater than 1 FNY' }));
                        }
            ...
            <Slide direction="right" in={true} mountOnEnter unmountOnExit {...{ timeout: 533 }}>
                <Box className="bond-data">
                    <div className="data-row">
                        ...
                        <p className="price-data bond-balance-title">{isBondLoading ? <Skeleton width="100px" /> : `${trim(bond.interestDue, 4)} FNY`}</p>
                    </div>
                    <div className="data-row">
                        <p className="bond-balance-title">Claimable Rewards</p>
                        <p className="price-data bond-balance-title">{isBondLoading ? <Skeleton width="100px" /> : `${trim(bond.pendingPayout, 4)} FNY`}</p>
                    </div>
                    <div className="data-row">
                        <p className="bond-balance-title">FNY until fully vested</p>
                        <p className="price-data bond-balance-title">{isBondLoading ? <Skeleton width="100px" /> : vestingMaia()}</p>
                    </div>
